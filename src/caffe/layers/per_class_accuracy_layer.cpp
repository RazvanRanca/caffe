...
void PerClassAccuracyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  Dtype* accuracyLabelPairs = accuracies_.mutable_cpu_data(); // accuracy i is in pos 2*i and label count i in position 2*i+1
  caffe_set(accuracies_.count(), Dtype(FLT_MIN), accuracyLabelPairs);
  
  for (int i = 0; i < num; ++i) {
    //count freq of each class
    accuracyLabelPairs[static_cast<int>(bottom_label[i])*2 + 1] += 1.0;
    ...
    if (max_id == static_cast<int>(bottom_label[i]))
      accuracyLabelPairs[static_cast<int>(bottom_label[i])*2] += 1.0;
  }
  ...
  //LOG(INFO) << "Accuracies, class by class: " << accuracyLabelPairs[0] << " out of " << accuracyLabelPairs[1];
  //can I do this or does 
  caffe_copy(accuracies_count, accuracyLabelPairs, top[0]->mutable_cpu_data());
